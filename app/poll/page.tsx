...
export default function PollPage() {
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      // Try MPC on-chain vote
      if (p.onchain_id) {
        setVotingStatus("Building encrypted vote...");
        const res = await fetch("/api/poll-vote", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            voterWallet: wallet,
            onchainProposalId: p.onchain_id,
            voteYes: sel === 0,
          }),
        });
        ...
        if (res.ok && data.transaction) {
          try {
            setVotingStatus("Waiting for wallet approval...");
            ...
            setVotingStatus("Confirming on-chain...");
            ...
            console.warn("On-chain vote failed, recording in database:", signErr.message);
          }
        } else {
          console.warn("MPC vote API failed:", data.error);
        }
      }

      // Record in Supabase
      setVotingStatus("Recording vote...");
  ...
  const handleComment = async () => {
    ...
    setPosting(true);
    try {
      const { data } = await supabase.from("poll_comments").insert({
        poll_id: Number(pollId),
        author: wallet,
        content: comment,
      }).select().single();
    ...
    <div className="min-h-screen bg-[#F7F5FB] dark:bg-[#110D20]">
      ...
      <div className="max-w-[700px] mx-auto px-6 py-10">
        <div className="flex gap-2 mb-5">
          <span className="px-3 py-1 rounded-lg bg-brand-purple-bg dark:bg-purple-500/10 text-brand-purple-dark dark:text-purple-300 text-[11px] font-semibold font-mono">⚡ Quick Poll</span>
          ...
          {p.onchain_id && <span className="px-3 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[11px] font-semibold font-mono">On-Chain</span>}
        </div>

        <h1 className="text-[28px] font-bold text-[#1A1625] dark:text-[#EEEAF6] mb-7">{p.question}</h1>

        <div className="p-5 rounded-[14px] bg-white dark:bg-[#1A1530] border border-[#E5E1EE] dark:border-[#2A2445] mb-5">
          <div className="flex justify-between mb-1.5">
            <span className="text-xs font-semibold text-[#4A4555] dark:text-[#A09BB0]">{p.votes || 0} votes cast</span>
          ...
          <div className="text-[11px] text-[#9C96AA] mt-1.5">🔒 Votes encrypted via Arcium MPC</div>
        </div>
        ...
        {!voted ? (
          <div className="p-6 rounded-2xl bg-white dark:bg-[#1A1530] border border-[#E5E1EE] dark:border-[#2A2445] mb-5">
            ...
            <div className="mt-3.5 p-3.5 rounded-xl bg-brand-purple-bg/30 dark:bg-purple-500/10 border border-brand-purple-bg dark:border-purple-500/20">
              ...
              <div className="text-xs text-[#6B6580] dark:text-[#A09BB0] leading-relaxed">Your vote is encrypted via multi-party computation on Solana. No one can see your choice.</div>
            </div>
          </div>
        ) : (
          <div className="p-6 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-300 dark:border-emerald-500/20 mb-5 text-center">
            ...
            <h3 className="text-[16px] font-bold text-emerald-800 dark:text-emerald-200 mb-1">Vote Recorded</h3>
            {mpcSuccess === true ? (
