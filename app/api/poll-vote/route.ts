...
export async function POST(req: NextRequest) {
  try {
    ...
    const { voterWallet, onchainProposalId, voteYes } = body;

    if (!voterWallet || onchainProposalId === undefined || voteYes === undefined) {
    ...
    if (!proposalInfo) {
      return NextResponse.json({ error: "Poll not found on-chain", onChain: false }, { status: 404 });
    }
    ...
    const sharedSecret = x25519.getSharedSecret(votePrivateKey, mxePublicKey) as Uint8Array;
