...
export async function POST(req: NextRequest) {
  try {
    ...
    const { creatorWallet, daoId, proposalId, title, description, optionA, optionB, deadlineHours } = body;

    if (!creatorWallet || daoId === undefined || proposalId === undefined || !title) {
    ...
    const daoPda = PublicKey.findProgramAddressSync(
      [Buffer.from("dao"), creatorPubkey.toBuffer(), new BN(daoId).toArrayLike(Buffer, "le", 4)],
      PROGRAM_ID
    )[0];
    ...
    if (!daoInfo) {
      return NextResponse.json({ error: "DAO not found on-chain", onChain: false }, { status: 404 });
    }
    ...
    return NextResponse.json({
      ...
      onChain: true,
      message: "Proposal creation ready for signing",
    });
  } catch (error: any) {
